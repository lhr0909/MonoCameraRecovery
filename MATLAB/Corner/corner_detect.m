function [cout,marked_img]=corner_detect(varargin)
...
%             fprintf('WTF\n');
%             disp(cout);
%             disp(ones(size(cout,1),1)*curve_start(i,:));
%             disp(size(cout));
%             disp(size(ones(size(cout,1),1)*curve_start(i,:)));
%             if (size(cout) ~= size(ones(size(cout,1),1)*curve_start(i,:)))
%                 cout = zeros(size(ones(size(cout,1),1)*curve_start(i,:)));
%             end
            if (size(cout, 1) == 0)
                break
            end
            compare_corner=bsxfun(@minus, cout, ones(size(cout,1),1)*curve_start(i,:));
